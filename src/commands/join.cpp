...
/*   Updated: 2024/09/29 15:40:28 by sshahary         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void Server::handleJoinCommand(int clientFd, const std::vector<std::string>& params) {
	...
	if (!channel) {
		...
		channel = new Channel(channelName);
		channels.push_back(channel); // Add it to the list of channels
	}
	...
	// Send the list of users currently in the channel (NAMES command response)
	std::string namesMessage = ":" + serverName + " 353 " + getClientNickname(clientFd) + " = " + channelName + " :" + channel->getUsersList(*this) + "\r\n";
	sendToClient(clientFd, namesMessage);
